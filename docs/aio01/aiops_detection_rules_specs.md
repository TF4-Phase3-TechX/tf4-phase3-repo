...
- **Status:** ✅ Rule specification completed and schema-verified; metric traffic verification and threshold calibration remain pending because the cluster was idle (2026-07-15)
...
Tài liệu này đặc tả chi tiết 3 quy tắc phát hiện sự cố (detection rules) phục vụ cho AIOps MVP Engine. Metric/log schema đã được đối chiếu với cụm EKS và source code; các ngưỡng hiện tại là baseline kỹ thuật ban đầu, chưa phải baseline production đã hiệu chuẩn. Mandate 07b phải chạy normal/incident windows để đo và điều chỉnh false positives.
...
- **Prometheus Metrics:** `app_llm_errors_total`, `app_llm_calls_total`, `app_llm_latency_seconds` và span calls metrics của product-reviews. Đây là contract hiện có trong source code production.

### Prometheus Alert Rule Spec (YAML)

Sử dụng provider-specific counter `app_llm_errors_total` để phát hiện lỗi Bedrock bị che bởi safe fallback. OpenSearch/trace evidence được dùng để phân loại timeout, throttling hoặc provider failure mà không log prompt/response:
  ...
  - name: aiops-llm-alerts
    rules:
      - alert: AIOpsLLMIntegrationFailureCritical
        # Kích hoạt khi tốc độ lỗi provider vượt 0.5 lỗi/giây.
        expr: |
        ...
        labels:
          ...
          team: aiops
        annotations:
          summary: "Critical LLM provider failure rate detected"
          description: "Bedrock/provider failures returning safe fallback exceeded 0.5 errors/s for 3 minutes."
...
| 4 | Verify live labels and sample rates for `app_llm_calls_total`, `app_llm_errors_total`, and `app_llm_latency_seconds` during the labeled #7b window | P1 | AIOps |
