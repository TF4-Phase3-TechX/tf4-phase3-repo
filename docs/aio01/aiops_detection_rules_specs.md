        ...
        expr: |
          histogram_quantile(
            ...
            sum by (le, service_name) (
              rate(traces_span_metrics_duration_milliseconds_bucket{service_name=~"product-reviews|checkout|cart"}[3m])
            )
          ) > 2000
        for: 3m
        labels:
    ...
    rules:
      - alert: AIOpsServiceErrorRateSpikeCritical
        expr: |
          (
            sum by (service_name) (
              rate(traces_span_metrics_calls_total{service_name=~"product-reviews|checkout|cart", status_code="STATUS_CODE_ERROR"}[2m])
            )
            /
            (sum by (service_name) (
              rate(traces_span_metrics_calls_total{service_name=~"product-reviews|checkout|cart"}[2m])
            ) > 0)
          ) * 100 > 5.0
          and on(service_name)
          sum by (service_name) (increase(traces_span_metrics_calls_total{service_name=~"product-reviews|checkout|cart"}[2m])) > 10
        for: 2m
        labels:
          severity: critical
          team: aiops
...
Phát hiện các lỗi đặc thù của luồng Bedrock trong `product-reviews`: provider
timeout, throttling/quota, IAM/Pod Identity, Guardrail/provider failure và
trường hợp AI Assistant có demand nhưng không có provider call thành công.

### Telemetry & Log Sources

- **OpenSearch Logs:** Log stream của `product-reviews` trong namespace
  `techx-tf4`, index `otel-logs-*`; legacy/injected `llm` service events are a
  separately classified drill signal, not the primary Bedrock contract.
- **Jaeger Traces:** Các trace span bị đánh dấu `ERROR` trên operation `AskProductAIAssistant`.
- **Prometheus Metrics:** Provider contract `app_llm_calls_total`,
  `app_llm_errors_total`, `app_llm_latency_seconds`; assistant demand
  `app_ai_assistant_counter_total` is an optional correlation guard.

### Prometheus Alert Rule Spec (YAML)

Sử dụng trực tiếp provider metrics do `product-reviews` phát ra. Span metrics
vẫn là evidence bổ trợ, nhưng không thay thế provider counters khi SDK fallback
có thể giữ application span ở trạng thái thành công.
  ...
  - name: aiops-llm-alerts
    rules:
      - alert: AIOpsLLMIntegrationFailureCritical
        ...
        expr: |
          sum(rate(app_llm_errors_total[3m])) > 0.5
          or
          (
            ((sum(rate(app_llm_calls_total[3m])) or vector(0)) == 0)
            and on()
      ...
      - alert: AIOpsLLMIntegrationFailureWarning
        expr: |
          sum(rate(app_llm_errors_total[3m])) > 0.1
        for: 3m
        labels:
          severity: warning
          team: aiops
...
1. **Controlled-injection classification:** `llmRateLimitError=true` may exercise
   the legacy/injected `llm` path rather than the Bedrock provider counters.
   Drill metadata and deployment window must identify it explicitly; do not
   suppress organic production failures with an assumed environment label.
2. **Cold-Start Noise & Time Window 3 Phút:** Giữ toàn bộ cửa sổ đánh giá ở
   `3m` và kết hợp demand guard để tránh coi rolling update/idle traffic là
   provider outage.
...
| 4 | Verify exported `app_llm_calls_total`, `app_llm_errors_total`, `app_llm_latency_seconds` buckets/labels and the assistant-demand counter under real AI traffic | P1 | AIOps |
