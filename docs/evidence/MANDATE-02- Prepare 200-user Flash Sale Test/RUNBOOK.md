...
- Load trigger: Locust Web UI qua service `load-generator` (không dùng Task-4 overlay/script)
...
|---|---|
| Launch mode | Locust Web UI; `LOCUST_HEADLESS=false`, `LOCUST_AUTOSTART=false` |
| Target | `200` users |
| Spawn rate | `3.33` users/s; đạt 200 users sau khoảng 60 giây |
| Traffic | API-only, browser traffic disabled |
| Target host | `http://frontend-proxy:8080` |
| Ramp-up | 60 giây |
| Steady-state | 900 giây / 15 phút, operator giữ run trong UI |
| Ramp-down | Operator chọn **Stop** trên Locust UI; ghi T1 UTC ngay lúc stop |
| Tổng runtime mục tiêu | 16 phút (60 giây ramp-up + 15 phút steady-state; không tính thời gian operator stop) |

> **Launch action:** Helm/CI chỉ deploy pod Locust ở trạng thái idle. **Start swarming** trên Locust UI là T0/start action. Mở toàn bộ UI, tạo evidence folder và ghi T0 ngay trước khi bấm Start; không chạy `kubectl exec ... locust` hoặc Task-4 script song song.
...
Chụp UI baseline theo `TASK-5-Pre-Load-Test-Baseline.md`, không tạo thêm file text. Trong Grafana Flash Sale Verification Dashboard, đặt range 30 phút ngay trước T0 và chụp các panel tương ứng vào `dashboard/` của RUN_ID. Giữ Grafana, Jaeger, Alertmanager, Grafana Explore/OpenSearch và Locust UI mở trước khi Start swarming; screenshot T0 phải được chụp ngay trước action này.

### Deploy Locust UI — chỉ khi CI chưa tạo pod

CI/Helm deploy image `6b5058f-load-generator` theo cấu hình mặc định: `LOCUST_HEADLESS=false`, `LOCUST_AUTOSTART=false`, `LOCUST_BROWSER_TRAFFIC_ENABLED=false`. Không apply Task-4 values. Trên cluster hai node, nếu cần rollout image mới, scale generator về 0 trước để tránh RollingUpdate tạo pod thứ hai cùng reservation `500m CPU / 768Mi` và bị `Insufficient cpu`.
...
# CI Helm apply image 6b5058f-load-generator ở đây.
kubectl -n "$NS" scale deployment/load-generator --replicas=1
kubectl -n "$NS" rollout status deployment/load-generator --timeout=180s
```

Xác nhận deployment có `replicas=1`, `LOCUST_HEADLESS=false`, `LOCUST_AUTOSTART=false`, `LOCUST_BROWSER_TRAFFIC_ENABLED=false` và target host `http://frontend-proxy:8080`. Các giá trị users/spawn rate được nhập trong UI ở mục 6, không sửa environment variables trong official run.
...
| Locust UI — trigger và raw result của official run | `http://localhost:18089/` | `kubectl -n techx-tf4 port-forward svc/load-generator 18089:8089` |

Mở Grafana trước để kiểm tra hai dashboard UID `flash-sale-verification` và `flash-sale-alert-state`, Jaeger để truy vấn trace, Grafana Explore với datasource OpenSearch (`webstore-logs`) để truy vấn `otel-logs-*`, rồi mở Locust UI. Không cần port-forward OpenSearch riêng: evidence logs được lấy qua Grafana OpenSearch datasource UI.
...
Tạo `<RUN_ID>`/evidence folder trước T0 và mở toàn bộ UI. Không chạy `kubectl exec ... locust` hay Task-4 script song song với Locust Web UI.
...
### Trigger official load trên Locust UI

1. Mở `http://localhost:18089/` và kiểm tra `Host` là `http://frontend-proxy:8080`.
2. Nhập **Number of users = 200** và **Spawn rate = 3.33**.
3. Chụp baseline UI, ghi T0 UTC, rồi bấm **Start swarming**. Đây là T0/start action.
4. Giữ run ít nhất 60 giây ramp-up và **15 phút steady-state**. Chụp checkpoint phút 1 / 7–8 / 14–15 theo mục 7.
5. Khi steady-state đủ 15 phút hoặc safety-stop condition xảy ra, ghi T1 UTC rồi bấm **Stop**. Không scale pod trước khi tải raw result.
6. Mở tab **Download Data** và tải CSV statistics, statistics history, failures, exceptions. Lưu tương ứng tại `locust/stats.csv`, `locust/stats-history.csv`, `locust/failures.csv`, `locust/exceptions.csv`. Tải HTML report từ UI nếu version UI cung cấp; nếu không có HTML report, ghi `BLOCKED` trong `SUMMARY.md` — không tạo report thay thế bằng CLI/script.

> **Raw-artifact gate:** `stats.csv`, `stats-history.csv`, `failures.csv`, `exceptions.csv` và `report.html` của đúng RUN_ID là bắt buộc để PASS. Nếu Locust UI không cung cấp một artifact bắt buộc, official acceptance là **BLOCKED**; dashboard/trace/log evidence không thay thế raw output.

Dùng Locust UI để xác nhận peak **200 active users**, duration và request/failure distribution. `stats-history.csv` là raw time-series chứng minh ramp-up/steady-state; CSV/HTML là raw load-generator output bắt buộc.
...
| T2 sau Stop | Load-generator activity trở về idle, final alert state, resource/restart/OOM/replica state |
