...
- **`DeleteConfigurationRecorder`, `StopConfigurationRecorder`, `DeleteDeliveryChannel`, `PutConfigurationRecorder`, `PutDeliveryChannel`**
  - **Vì sao nguy hiểm:** Xóa, dừng hoặc sửa recorder/delivery channel có thể làm mất change trail, thu hẹp phạm vi ghi nhận hoặc chuyển evidence sang đích khác. Tất cả kích hoạt báo động `CRITICAL`.
...
Để tránh alert fatigue mà không tạo điểm mù, Lambda không bỏ qua toàn bộ sự kiện chỉ dựa trên tên actor. Allowlist phải khớp đồng thời actor, API và resource:

- `SecuritySlackAlertsLambdaRole` chỉ được bỏ qua khi gọi `GetParameter` cho đúng `/security-alerts/slack-webhook-url`.
- `tf4-github-actions-terraform-apply` và `tf4-github-actions-plan` chỉ được bỏ qua khi gọi `GetParameter`/`GetParameters` và toàn bộ tên parameter đều là `/security-alerts/slack-webhook-url`. `GetParametersByPath` không được allowlist.
- `external-secrets-techx-tf4-cluster` chỉ được bỏ qua khi gọi `GetSecretValue` cho secret thuộc prefix `techx/tf4/`.
- `techx-tf4-postgresql-dms-secrets-access` chỉ được bỏ qua khi session là `dms-session-for-replication-engine` và gọi `GetSecretValue` cho một trong hai secret migration `techx/tf4/dms-postgres-source-*` hoặc `techx/tf4/rds-postgres-*`.
- `karpenter-controller-techx-tf4-cluster` chỉ được bỏ qua khi gọi `GetParameter` cho public parameter thuộc `/aws/service/eks/optimized-ami/`.
- AWS MSK chỉ được bỏ qua khi `userIdentity.type=AWSService`, `invokedBy`, source IP và user agent đều là `kafka.amazonaws.com`, đồng thời secret ARN thuộc đúng account/Region và có prefix `AmazonMSK_`.
- Security Group chỉ bị cảnh báo khi ingress mở ra `0.0.0.0/0` hoặc `::/0`.
- S3 policy/ACL chỉ bị cảnh báo khi thay đổi cấp quyền public.
- Critical event luôn cảnh báo, không phụ thuộc actor.
- Các write event đặc quyền từ `tf4-github-actions-terraform-apply` vẫn gửi Slack và được ghi rõ là thay đổi CI/CD. Pipeline bị chiếm quyền cũng dùng chính role này nên không được suppress.

Các sự kiện được bỏ qua vẫn còn trong CloudTrail; Lambda ghi lý do filter vào CloudWatch Logs.

### Kết quả rà soát runtime ngày 20/07/2026

Trong 48 giờ, CloudTrail ghi nhận 893 event thuộc tập đang quan sát. Nguồn nhiễu hợp lệ lớn nhất là 556 lần External Secrets đọc `techx/tf4/*`, 60 lần Alert Lambda đọc đúng webhook, 110 lần Terraform plan/apply đọc đúng webhook, 6 lần Karpenter đọc public EKS AMI parameter và 3 lần MSK đọc secret `AmazonMSK_*`. Các trường hợp này là phạm vi exact allowlist.

Các lần người dùng SSO đọc secret/parameter, các IAM write từ pipeline và ingress public vẫn cảnh báo. Đây là chủ ý để không tạo điểm mù cho tài khoản người dùng, pipeline bị chiếm quyền hoặc cấu hình mạng nguy hiểm.

Ngày 20/07/2026, DMS migration engine phát sinh bốn lần `GetSecretValue` hợp lệ cho hai secret PostgreSQL và bị cảnh báo trước khi có exact allowlist. Event `2ec088ec-9ced-4a61-a2f2-55e2fe057e58` đã tới Slack sau 2.552 giây; đây là baseline để xác minh không còn notification sau deploy.
