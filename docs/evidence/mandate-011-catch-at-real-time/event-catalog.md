...
- **Lọc theo Actor (CI/CD & Internal Agents):** Nếu ARN của `actor` có chứa chuỗi `role/tf4-github-actions` (CI/CD) hoặc `external-secrets` (Kubernetes agent), Lambda sẽ chủ động bỏ qua (log lại vào CloudWatch nhưng không bắn thông báo Slack). Các thao tác từ pipeline CI/CD và internal system agents được coi là an toàn và đã được rà soát từ source code.
