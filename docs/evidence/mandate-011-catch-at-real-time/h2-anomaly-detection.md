...
**Branch:** `cdo07/fix/anomaly-detection` | **Commits:** `93da98b`, `ef4bb03`, `69aa440`
...
CloudTrail
    │
    ▼
EventBridge: cloudtrail-readonly-sensitive (existing)
    │ state = ENABLED_WITH_ALL_CLOUDTRAIL_MANAGEMENT_EVENTS
    ▼
SNS: audit-security-alerts (existing)
    │
    ▼
Lambda: audit-security-slack-alerts (existing — extended)
    │
    ├── [H1] is_expected_sensitive_read() → log MANDATE11_EXPECTED_READ
    │         (allowlisted calls — vẫn giữ nguyên, không thay đổi)
    │
    └── [H1+H2] publish_latency_metrics() → log MANDATE11_TTD
              { marker, eventName, pipeline, eventId, latencySeconds, ... }
              (MỌI event được xử lý, kể cả allowlisted)
    │
    ▼
CloudWatch Logs: /aws/lambda/audit-security-slack-alerts (retention: 365 ngày)
    │
    ├── Metric Filter A: { $.marker = "MANDATE11_TTD" && $.eventName = "GetSecretValue" }
    │         → Namespace: Mandate11/AllowlistActivity
    │         → MetricName: GetSecretValueTotalCount
    │         → Value: 1, Default: 0, Unit: Count
    │
    └── Metric Filter B: { $.marker = "MANDATE11_EXPECTED_READ" }
              → Namespace: Mandate11/AllowlistActivity
              → MetricName: ExpectedReadCount
              → Value: 1, Default: 0, Unit: Count
    │
    ▼
CloudWatch Metric: Mandate11/AllowlistActivity
    │
    ├── Alarm A [STATIC THRESHOLD]
    │     mandate11-get-secret-value-rate-spike
    │     GetSecretValueTotalCount > 10 / period=60s / eval=1 / datapoints=1
    │     → ALARM trong <2 phút khi có spike
    │
    ├── Alarm B [ANOMALY DETECTION]
    │     mandate11-get-secret-value-anomaly-detection
    │     ANOMALY_DETECTION_BAND(m1, 2) / period=300s / eval=2
    │     m1.return_data=false / ad1.return_data=true
    │     → ALARM khi slow exfil pattern vượt ML band
    │
    └── Alarm C [DEAD-MAN'S SWITCH]
          mandate11-pipeline-silence-detection
          DetectionLatencySeconds SampleCount < 1 / period=43200s (12h)
          treat_missing_data=breaching
          → ALARM khi pipeline im lặng >12h
    │
    ▼
SNS: audit-security-alerts-anomaly (NEW — KMS encrypted)
    │ Policy: cloudwatch.amazonaws.com được Publish
    │
    ├── Subscription: email → alert_email_endpoint (backup khi Lambda down)
    │
    └── Subscription: lambda → audit-security-slack-alerts
    │
    ▼
Lambda: audit-security-slack-alerts (extended)
    │ Detect payload: 'AlarmName' in message && 'NewStateValue' in message
    │ → handle_cloudwatch_alarm()
    │     rate-spike: color=#ff0000, severity=CRITICAL
    │     anomaly:    color=#ff9900, severity=HIGH
    │     OK state:   skip (không alert)
    ▼
Slack webhook → Security Team channel
```
