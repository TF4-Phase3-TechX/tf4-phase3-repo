...
> [!WARNING]
> **Giới hạn của Snapshot:** Dữ liệu `kubectl top` trên chỉ là ảnh chụp tức thời (instant snapshot) tại một thời điểm baseline. Mọi nhận định về rủi ro OOMKilled và co giãn Node (Autoscaling) trong báo cáo này được đề xuất dưới dạng khuyến nghị kỹ thuật phòng ngừa (precautionary recommendations). Do phiên chạy Locust chính thức trong approved maintenance window hiện ghi nhận là NOT RUN (EXECUTION GATED), các suy luận này chưa được xác thực bằng dữ liệu load test time-series thực tế và cần được đội ngũ vận hành thực nghiệm kiểm chứng thêm.
...
Việc duy trì các giới hạn RAM limit an toàn cho các Pod như `frontend` (320Mi) và `product-reviews` (192Mi), cũng như đề xuất biên an toàn cho `checkout` (60Mi) là quyết định kỹ thuật nhằm **nâng cao tính ổn định**, hoàn toàn không phải để giảm chi phí. Khoảng đệm RAM đảm bảo độ tin cậy giúp loại bỏ nguy cơ sập Pod bất ngờ khi runtime thực thi dọn rác (Garbage Collection) dưới tải cao.
