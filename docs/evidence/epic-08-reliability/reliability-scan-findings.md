...
- Observability/SLO evidence path.
- Alerting.
...
| REL-04 | Observability stack hiện không available trong `techx-tf4` | `grafana`, `prometheus`, `jaeger`, `opensearch`, `otel-collector` services `NotFound`; events có `Killing pod/grafana`, `Killing pod/prometheus`, `Killing pod/opensearch` | CDO08 không có đường evidence ổn định để chứng minh SLO/trace/log khi review hoặc pitch | P1 | Khôi phục hoặc xác nhận namespace observability, rồi validate checkout SLO queries |
...
### REL-04 - Observability unavailable

Commands:

```bash
kubectl -n techx-tf4 get svc grafana prometheus jaeger opensearch otel-collector
kubectl -n techx-tf4 get events --field-selector type=Warning --sort-by=.lastTimestamp
```

Kết quả:

- Services observability không tồn tại trong namespace `techx-tf4` tại thời điểm scan.
- Events có evidence các pod observability bị killing hoặc readiness failed gần thời điểm deploy.

Việc cần làm tuần sau:

- Quyết xác nhận observability stack đang nằm namespace nào.
- Nếu Grafana/Prometheus đang sập, tạo issue khôi phục.
- Validate checkout SLO queries sau khi observability hoạt động ổn định.
...
| REL-BL-04 | Restore/validate observability stack and checkout SLO evidence path | Quyết | P1 |
...
Reliability Week 1 scan đã xác nhận hệ thống app hiện đang chạy, nhưng resilience baseline còn yếu: single replica, thiếu probes, stateful components chưa có PVC/evidence backup, observability/alerting chưa ổn định, và checkout có consistency risk khi Kafka publish fail sau payment/shipping. Các issue này phù hợp để đưa vào backlog tuần sau theo priority P1/P2.
