...
1. **Chuyển đổi 100% sang ARM64**: Toàn bộ worker nodes của EKS (cả On-Demand và Spot) được chuyển sang kiến trúc ARM64 (	4g.large, c7g.large, c7g.xlarge, 
7g.large).
2. **Approved On-Demand Reliability Floor (Ngưỡng On-Demand được duyệt)**:
   - 2 Managed ARM64 	4g.large On-Demand nodes phân bổ trên 2 AZ (us-east-1a và us-east-1b) làm nền tảng bootstrap & daemon điều khiển cluster.
   - 1 Protected ARM64 	4g.large On-Demand node (us-east-1b) dành riêng cho OpenSearch và hệ thống observability lưu trữ dữ liệu (gắn volume EBS RWO AZ-bound).
