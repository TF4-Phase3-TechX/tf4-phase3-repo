...
| CDO08-REL-04 | Epic 08 | Reliability | P1 | Quyết | Observability stack hiện không available trong `techx-tf4` | CDO08 không có đường evidence ổn định để chứng minh SLO/trace/log khi review hoặc pitch | `kubectl -n techx-tf4 get svc grafana prometheus jaeger opensearch otel-collector` đều `NotFound`; events có `Killing pod/grafana`, `Killing pod/prometheus`, `Killing pod/opensearch` | Xác nhận observability stack nằm namespace nào hoặc đang sập; khôi phục/validate access | Grafana/Prometheus/Jaeger/OpenSearch/OTel có service/pod healthy hoặc có namespace/service chính xác | Nếu restore observability gây resource pressure, rollback chart values/deploy stamp | CDO04 nếu cần resource; Deploy Operator nếu cần deploy lại |
...
|---|---|
| Observability stack hiện nằm namespace nào, hay thật sự đã sập/gỡ? | Quyết |
