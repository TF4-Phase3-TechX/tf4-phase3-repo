...
| :--- | :--- | :--- | :--- | :--- |
| **ĐÃ CHỌN** | `B` | Provisioned với broker nhỏ (`kafka.t3.small` × 2, 1 broker/AZ, RF=2 tăng từ RF=1 hiện tại) đáp ứng đúng workload thực tế: **chỉ 1 topic `orders`**, throughput thấp (order event, không phải high-frequency stream). Không cần auto-scale linh hoạt như Serverless vì traffic pattern ổn định, dễ dự đoán. RF=2 (thay vì 3-broker RF=3 tốn hơn) là cân bằng hợp lý: cải thiện so với RF=1 in-cluster hiện tại mà không tốn kém như cấu hình full-HA. | **Verified qua AWS Price List API (2026-07-17):** `kafka.t3.small` = **$0.0456/broker-giờ** (SKU `EAENBG6PQA5U8JTN`), storage = **$0.10/GB-tháng** (SKU `65XV7TSGW9ZJSRYZ`). 2 broker × 730h × $0.0456 = **$66.58/tháng** broker cost + storage 20-40GiB (10-20GiB/broker) × $0.10 = **$2-4/tháng** → **Tổng $68.6-70.6/tháng (~$15.8-16.3/tuần)**. **Đây là số đã fact-check và ĐIỀU CHỈNH TĂNG** so với ước tính gốc (`~$61-67/tháng`, kế thừa từ REL-03 `$61.04/mo`) — số cũ thấp hơn thực tế khoảng **$5-9/tháng (~10-13%)**, không có nguồn AWS Pricing gốc nào backing nó (khác EC2 t3.large đã có evidence JSON). Đây vẫn là **cấu phần đắt nhất** trong migration (~70% phần chi phí tăng thêm so với baseline `~$56.83/tuần`), nằm trong trần `$300/tuần` (tổng sau migration ước tính ~`$79/tuần`, ~26% budget) nhưng cần giải trình rõ nhất với CDO04. **Worst-case cost risk (chưa từng nêu trước đây):** `kafka.t3.small` là **burstable instance** (mô hình CPU credit, AWS khuyến nghị chỉ cho dev/test hoặc low-throughput có burst tạm thời). Nếu traffic thật (hoặc Locust load-test) làm cạn CPU credit, đường nâng cấp là `kafka.m7g.large` = **$0.204/broker-giờ** (verified cùng nguồn) → 2 broker × 730h = **$297.84/tháng (~$68.6/tuần) chỉ riêng broker cost** — gấp **~4.3 lần** kịch bản t3.small. Cần báo trước cho CDO04 đây là ngưỡng escalation, không phải chi phí cố định. | **Bắt buộc CDO04 duyệt riêng** trước khi provision — đây là quyết định reopen lại chỗ CDO04 từng defer MSK ở REL-03 (`docs/cdo08/week2/mandate3/review-requests/REVIEW-REQUEST-CDO04-COST-REL03-HA-OPTIONS.md`). Cần làm rõ với CDO04: (1) trần `$300/tuần` có được nới cho Directive #8 không (BUDGET.md cho phép BTC nới khi ban hành directive lớn bắt buộc migration), (2) nếu MSK vượt duyệt thì escalate BTC vì Mandate 8 là bắt buộc — không thể tự ý chọn Phương án C, (3) chấp nhận ngưỡng escalation lên `kafka.m7g.large` (~$68.6/tuần broker cost) nếu t3.small cạn CPU credit dưới traffic thật. |
| **BỊ LOẠI BỎ** | `A` | MSK Serverless có ưu điểm auto-scale, không cần quản lý broker/capacity — nhưng **base cost $0.75/giờ/cluster = $547.50/tháng** (730h × $0.75, verified) chỉ riêng phí nền — **cao hơn** con số gốc `~$540/tháng` đã nêu. Cộng thêm partition-hour `$0.0015/partition-giờ` (~$3.3/tháng cho 3 partition) + data in/out (`$0.10/GB-in`, `$0.05/GB-out`) + storage `$0.10/GB-tháng` — dù các phần sau nhỏ với throughput thấp của `orders`, tổng **~$550-560/tháng (~$127-129/tuần)** — vỡ hoàn toàn trần `$300/tuần` cho một workload chỉ 1 topic, throughput thấp. Trả tiền cho khả năng co giãn mà hệ thống hiện tại không cần. | **~$550-560/tháng (~$127-129/tuần)** (verified + worst-case add-on, xem cột trước) — đắt hơn Provisioned ~8 lần cho cùng workload — không hợp lý về mặt cost-efficiency (nguyên tắc chấm điểm theo BUDGET.md). | Sẽ cần xin duyệt ngân sách ngoại lệ lớn — không có cơ sở hợp lý để trình khi Provisioned đã đáp ứng đủ nhu cầu với chi phí thấp hơn nhiều. |
...
* **Fact-check cost (2026-07-17):** số MSK ở KF-01 đã được verify trực tiếp qua AWS Price List API (`pricing.us-east-1.amazonaws.com/offers/v1.0/aws/AmazonMSK/...`), không dùng số ước lượng kế thừa từ REL-03. Phát hiện: `docs/audit/adr/004-in-cluster-datastores-week1-baseline.md` ước tính MSK **~$50-100/tuần** — cao hơn 3-6 lần số verified (~$15.8-16.3/tuần) — không có nguồn AWS Pricing nào backing, nên coi đây là placeholder cũ chưa cập nhật, **không dùng** cho quyết định approve/reject; số ở KF-01 (bottom-up từ broker-hour × storage, có SKU cụ thể) là số nên dùng để trình CDO04.
