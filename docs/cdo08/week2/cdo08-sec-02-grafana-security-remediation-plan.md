...
> The Kubernetes Secret `grafana-admin-creds` **MUST** be created in the cluster *before* the Helm chart is upgraded/deployed. If the Secret does not exist, the Grafana pods will fail to start and enter a `CreateContainerConfigError` state because they cannot mount the referenced secret keys.

*(Thực hiện trước khi deploy Helm - Sử dụng lệnh Idempotent)*
```bash
# Tạo secret chứa cả admin-user và admin-password dạng idempotent
...
* `techx-corp-platform/src/grafana/grafana.ini`
* `techx-corp-platform/docker-compose.yml`
* `techx-corp-platform/docker-compose.minimal.yml`
...
```env
GRAFANA_ADMIN_PASSWORD=<set-in-local-env-not-committed>
```
...
### 7.1. Kiểm tra lớp 1: Public ALB (Public access must be blocked)
Đảm bảo rằng Grafana không khả dụng qua route public của Ingress ALB (theo đúng Mandate #1 / SEC-05):
```bash
# Public path phải trả về 404 hoặc bị từ chối truy cập (không trả về 200)
curl -I http://<alb-ingress-dns>/grafana/
```

### 7.2. Kiểm tra lớp 2: Private access & Anonymous Permission
Thực hiện truy cập nội bộ (qua Port-forward/SSM tunnel) để kiểm chứng cấu hình phân quyền anonymous:
```bash
# Port-forward service Grafana lên cổng local 3000
kubectl -n techx-observability port-forward service/grafana 3000:80

# Truy vấn API home dashboard bằng quyền nặc danh qua localhost
curl -s http://localhost:3000/grafana/api/dashboards/home
```

### Điều kiện Pass:
- [ ] Truy cập qua Public ALB: Nhận phản hồi `404 Not Found` hoặc bị timeout/chặn từ ngoài.
- [ ] Truy cập qua Port-forward: Lệnh curl API nặc danh trả về mã code `401 Unauthorized` / `403 Forbidden`, hoặc trả về thành công `200 OK` nhưng JSON chứa `.meta.canEdit = false` (chỉ có quyền Viewer, không thể lưu thay đổi).
...
1. **Before:** Cung cấp bằng chứng từ cấu hình code (`values.yaml` có `auth.anonymous.org_role: Admin` và `adminPassword: admin`). Không yêu cầu chụp ảnh qua public `/grafana` nếu public route này hiện đã bị SEC-05 chặn truy cập từ ngoài.
2. **Change:** Link pull request chứa các file cấu hình đã xóa password hardcode và hạ quyền anonymous.
3. **After:** Ảnh chụp màn hình khi truy cập nặc danh (chỉ có Viewer qua local port-forward) và sau khi đăng nhập admin (có đầy đủ quyền Admin).
