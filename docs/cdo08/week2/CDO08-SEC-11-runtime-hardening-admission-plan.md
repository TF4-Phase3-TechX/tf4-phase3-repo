...
kind: ValidatingAdmissionPolicy
metadata:
  name: sec11-drop-all-capabilities
spec:
  failurePolicy: Fail
  matchConstraints:
    resourceRules:
...
### 5.3 Binding — production AUDIT + test namespace DENY

Thiết kế 2 nhóm binding cho **cùng 4 policy**:
- **Nhóm PRODUCTION** (`techx-tf4`, `techx-observability`): cả 4 luật `[Audit, Warn]` — **không chặn** workload đang chạy. Đây là trạng thái tuần này ("chưa deny ngay").
- **Nhóm DEMO** (namespace cô lập `techx-admission-test`): cả 4 luật `[Deny]` — để mentor apply manifest vi phạm và thấy **reject thật** (yêu cầu mandate), không đụng production.

#### Production bindings — tất cả AUDIT

```yaml
# 4 binding, KHÁC NHAU ở name + policyName, GIỐNG NHAU phần còn lại.
apiVersion: admissionregistration.k8s.io/v1
kind: ValidatingAdmissionPolicyBinding
metadata: { name: sec11-no-root-containers-binding }
spec:
...
apiVersion: admissionregistration.k8s.io/v1
kind: ValidatingAdmissionPolicyBinding
metadata: { name: sec11-drop-all-capabilities-binding }
spec:
  policyName: sec11-drop-all-capabilities
  validationActions: [Audit, Warn]   # chờ SEC-09 (#233) rollout xong
  matchResources:
    namespaceSelector:
      matchLabels: { techx.io/policy-scope: enforced }
...
apiVersion: admissionregistration.k8s.io/v1
kind: ValidatingAdmissionPolicyBinding
metadata: { name: sec11-no-mutable-image-tag-binding }
spec:
...
apiVersion: admissionregistration.k8s.io/v1
kind: ValidatingAdmissionPolicyBinding
metadata: { name: sec11-resources-required-binding }
spec:
...
#### Demo bindings — DENY, chỉ trong `techx-admission-test`

```yaml
# Lặp cho cả 4 policy — chỉ đổi name (hậu tố -deny) + policyName. Ví dụ 1 cái:
apiVersion: admissionregistration.k8s.io/v1
kind: ValidatingAdmissionPolicyBinding
metadata: { name: sec11-no-mutable-image-tag-deny }
spec:
  policyName: sec11-no-mutable-image-tag
...
# ... 3 binding tương tự cho sec11-no-root-containers / sec11-drop-all-capabilities /
#     sec11-resources-required, cùng namespaceSelector techx-admission-test.
```
...
| 2 | Apply 4 `ValidatingAdmissionPolicy` (§5.2) | `kubectl get validatingadmissionpolicy` thấy đủ 4, `status.conditions` không lỗi |
| 3 | Apply 4 binding **production** `[Audit, Warn]` (§5.3) | Không chặn workload nào; audit annotation bắt đầu ghi nhận vi phạm |
| 4 | Apply 4 binding **demo** `[Deny]` scope `techx-admission-test` (§5.3) | Sẵn sàng cho mentor apply manifest vi phạm → thấy reject thật |
| 5 | **Full sweep** ghi nhận vi phạm hiện tại theo từng luật (input cho §6.1) | Report: tag còn busybox init (6 svc), root/caps còn nhiều, resources = 0 |
| 6 | Thông báo tf4-leads/Nguyên: admission đã lên **audit-only** trên production, demo Deny ở test ns | Có xác nhận review; không ai bị chặn |
...
3 file test, namespace **`techx-admission-test`** (namespace cô lập có binding `Deny` — §5.3), dùng image công khai (`nginx`) để không phụ thuộc registry nội bộ. Áp vào đây để thấy **reject thật** mà không chạm production:
...
- [ ] 4 `ValidatingAdmissionPolicy` + 4 binding production (Audit) + 4 binding demo (Deny, scope `techx-admission-test`) apply thành công
