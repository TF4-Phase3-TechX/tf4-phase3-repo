...
Artifact `security-artifacts-<tag>` trên GitHub Actions run tương ứng chứa:
- `trivy-checkout-sha256-<digest_clean>.txt` — table format (human readable)  
- `trivy-checkout-sha256-<digest_clean>.json` — JSON format (machine readable)
...
gh run download <run-id> \
  ...
  --name security-artifacts-8340af1

# Kiểm tra scan result
cat trivy-checkout-*.txt
# Expected: "Total: 0" hoặc không có dòng HIGH/CRITICAL, hoặc
# nếu có thì build đã fail (không thể promote)

# Scan lại bất kỳ lúc nào từ digest
DIGEST="sha256:<lấy từ subtask 1>"
...
```bash
# Thay <digest> bằng sha256 lấy từ Subtask 1
...
**Expected:** JSON với `repo`, `commit`, `workflow`, `run_id`, `service_name`, `image_digest` rõ ràng.

### Evidence artifact

File `provenance-verify-checkout-sha256-<digest_clean>.txt` trong artifact `security-artifacts-<tag>`.
...
- `sbom-checkout-sha256-<digest_clean>.json` — raw SBOM file  
- `sbom-verify-checkout-sha256-<digest_clean>.txt` — verify output  
(cả hai trong artifact `security-artifacts-<tag>`)
