...
## 6. Actual Admission Test Output — chạy thật 2026-07-20T18:38:20+07:00

**Timestamp:** `2026-07-20T18:38:20+07:00`  
**Namespace:** `techx-sec17-admission-test`  
**Labels active:**
```
NAME                         STATUS   AGE   LABELS
techx-sec17-admission-test   Active   84m   kubernetes.io/metadata.name=techx-sec17-admission-test,techx.io/sec17-digest-enforce=true,techx.io/sec17-signature-enforce=true
```

### Test 1 — Tag-only image `techx-corp:8340af1-checkout` (Expected: REJECTED)

**Policy triggered:** `require-digest-image-reference` (ValidatingAdmissionPolicy — native K8s)

```
The pods "sec17-tag-only" is invalid: : ValidatingAdmissionPolicy
'require-digest-image-reference' with binding 'require-digest-image-reference-binding'
denied request: Image must be pinned by immutable digest:
...
`8340af1-checkout` build trước PR #293 (SEC-15, 2026-07-18) — không có cosign signature.  
...
denied the request: Policy require-signed-techx-images error:
failed to evaluate policy: GET https://511825856493.dkr.ecr.us-east-1.amazonaws.com/
v2/techx-corp/manifests/sha256:a4d43a5b0cc0fe03cc0ed3435d87fb28e1e469af7f4c37c8390eb9f8f22f16f9:
unexpected status code 401 Unauthorized: Not Authorized
```

**Result: ✅ REJECTED** — Kyverno active và fail-closed. `failurePolicy: Fail` đúng spec — deny khi không verify được, bảo vệ cluster khỏi unverified image.

---

### Test 3 — Signed image với digest `techx-corp:411e9a2-currency@sha256:0c910c26...` (Expected: ALLOWED — pending IRSA)

```
Error from server: error when creating "sec20-test-valid.yaml":
admission webhook "ivpol.validate.kyverno.svc-fail-finegrained-require-signed-techx-images"
denied the request: Policy require-signed-techx-images error:
failed to evaluate policy: GET https://511825856493.dkr.ecr.us-east-1.amazonaws.com/
v2/techx-corp/manifests/sha256:0c910c26005a088d406bf6fe3ad34f52dc3cccd06656ff4d3a7303549ac005dc:
unexpected status code 401 Unauthorized: Not Authorized
```

**Result: ⚠️ REJECTED** — Kyverno ECR 401. Kyverno service account chưa có IRSA để authenticate ECR khi pull cosign signature tag.  
**Root cause:** Infrastructure gap — cần cấu hình IRSA hoặc `imagePullSecret` cho Kyverno pod.  
**Implication:** Khi IRSA được cấu hình, image `411e9a2-currency` (đã có cosign signature từ `build-and-push.yaml@main`) sẽ pass Test 3.
...
|---|---|---|---|---|
| Tag-only | `8340af1-checkout` (no digest) | VAP `require-digest-image-reference` | ✅ REJECTED | Digest policy hoạt động đúng |
| Unsigned+digest | `8340af1-checkout@sha256:a4d4...` | Kyverno `require-signed-techx-images` | ✅ REJECTED | Fail-closed, deny unverifiable image |
| Signed+digest | `411e9a2-currency@sha256:0c91...` | Kyverno `require-signed-techx-images` | ⚠️ REJECTED | Kyverno cần IRSA cho ECR — tracked gap |
