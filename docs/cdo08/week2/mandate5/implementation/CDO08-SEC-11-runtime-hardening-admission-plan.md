...
- **Trạng thái enforce hiện tại:** cả 4 binding chuyển sang `[Deny]` trong PR này (branch `cdo08/sec-11-argocd-resource-fix-evidence`) — xem §6, §6.1 cho điều kiện merge/gate.
...
- `external-secrets`: **đã bổ sung resources xong** (PR [#33](https://github.com/TF4-Phase3-TechX/tf4-phase3-gitops-manifests/pull/33), right-size theo review CDO-04 dựa trên Prometheus 3 ngày) — sạch cả 4 luật.
- `argocd`: **đã bổ sung resources cho 9/9 component chính** qua `helm upgrade` thủ công (evidence: `docs/cdo08/week2/mandate5/evidence/CDO08-SEC-11-argocd-resource-fix-evidence.md`) — nhưng phát sinh 1 gap mới chưa cover trong đề xuất ban đầu: Helm hook Job `argocd-redis-secret-init` (container `secret-init`) vẫn `resources: {}` rỗng, gây đúng 2 audit + 2 warn hit trên `require-resource-limits` mỗi lần `helm upgrade`/install chạy. Cần chạy thêm 1 lệnh `helm upgrade --reuse-values --set redisSecretInit.resources.*` (đã verify key này dùng trực tiếp, không bị chart bỏ qua như trường hợp `repoServer.copyutil`) **trước khi** merge/sync PR chuyển `[Deny]` — xem §6, §10.
...
| `argocd` | 8/9 | Đã bổ sung `resources` cho 9 component chính (Deployment/StatefulSet) qua `helm upgrade` — chỉ còn Job hook `argocd-redis-secret-init` (`secret-init`) chưa có `resources`. Xem §6/§10. |
| `external-secrets` | 3/3 | Đã bổ sung `resources` xong qua PR #33 (right-size theo review CDO-04). |
...
**`validationActions` chuyển sang `[Deny]`** trong PR này — điều kiện merge/gate: xem §6.
...
| 1 | Merge PR [#302](https://github.com/TF4-Phase3-TechX/tf4-phase3-repo/pull/302) (`tf4-phase3-repo`) — scope cluster-wide + fix regex tag+digest | ✅ **Đã merge vào `main`** |
| 2 | Bổ sung resources cho `external-secrets` — PR [#31](https://github.com/TF4-Phase3-TechX/tf4-phase3-gitops-manifests/pull/31) → right-size lại theo review CDO-04 ở PR [#33](https://github.com/TF4-Phase3-TechX/tf4-phase3-gitops-manifests/pull/33) (repo gitops) | ✅ **Đã xong** |
| 3 | Bổ sung resources cho `argocd` — lệnh `helm upgrade` thủ công (không quản lý qua GitOps, xem §10). Evidence: `docs/cdo08/week2/mandate5/evidence/CDO08-SEC-11-argocd-resource-fix-evidence.md` | ✅ **Đã chạy — 9/9 component chính có resources.** ⚠️ Phát sinh gap mới: Job hook `argocd-redis-secret-init` (không nằm trong đề xuất ban đầu) vẫn `resources: {}` |
| 3b | Bổ sung resources cho Job hook `argocd-redis-secret-init` — `helm upgrade --reuse-values --set redisSecretInit.resources.*` (đã verify key hợp lệ trực tiếp trong chart, không bị bỏ qua) | ⏳ **Đang chờ chạy — BẮT BUỘC trước khi merge/sync bước 4** |
| 4 | Merge PR này (`cdo08/sec-11-argocd-resource-fix-evidence`, gồm evidence + flip `[Deny]`) → bot promote tạo PR bump chart revision bên gitops → merge PR đó → ArgoCD tự sync | Chỉ merge sau khi bước 3b xong và verify lại 0 audit/warn mới |
| 5 | Verify: `kubectl get validatingadmissionpolicy,validatingadmissionpolicybinding` — đủ 8 object, `[Deny]` | Sau bước 4 |
| 6 | Quan sát hệ thống ổn định qua kênh audit/warn → giờ là kênh Deny/ArgoCD sync-fail (§6.2) | Sau bước 5 |
| 7 | ~~Chuyển `validationActions` từ `[Audit, Warn]` sang `[Deny]`~~ — đã gộp vào bước 4 của PR này | ✅ |
| 8 | Thông báo tf4-leads/Nguyên: admission đã enforce `[Deny]` cả 4 luật trên production | Sau bước 6 |

### 6.1 Rủi ro compliance tại thời điểm bật Deny
...
| Drop capabilities | ✅ | ✅ | ✅ | ✅ |
| Image tag | ✅ | ✅ | ✅ | ✅ |
| Resource limits | ✅ | ✅ | ⚠️ **9/9 component chính ✅, Job hook `argocd-redis-secret-init` ❌ (pending §6 bước 3b)** | ✅ |

**Điều kiện merge PR này:** bước 3b (§6) phải chạy xong và verify lại `apiserver_validating_admission_policy_check_total{enforcement_action=~"warn|audit"}` không tăng thêm ở lần `helm upgrade` kế tiếp, trước khi merge/để ArgoCD sync bản chart có `[Deny]` — nếu không, lần `helm upgrade`/`install` argocd tiếp theo (dù vì lý do gì khác) sẽ bị chặn cứng ngay tại Job hook này.
...
**Khi còn `[Audit, Warn]`:** cả 3 manifest test **KHÔNG bị reject** — chỉ hiện `Warning:` kèm message tương ứng, và audit log/metric (§6.2) ghi nhận vi phạm. Không nhầm với "chưa hoạt động" — đây đúng hành vi Audit/Warn theo thiết kế K8s (không có Event, không fail request).

**Sau khi merge/sync PR chuyển sang `[Deny]` (§6 bước 4-5, gate ở §6.1):** cả 3 manifest test phải bị **reject thật**, kèm message đúng luật vi phạm — đây mới là bằng chứng cuối cùng để nộp cho mentor theo yêu cầu mandate ("apply thử một manifest vi phạm và thấy bị từ chối"). Cần chạy lại 3 lệnh dry-run này sau khi Deny có hiệu lực để lấy evidence reject thật, cập nhật vào `MANDATE-05-RUNTIME-HARDENING-EVIDENCE.md`.
...
- Trạng thái: chuyển `[Deny]` sau khi `argocd`/`external-secrets` có resources đầy đủ (kể cả Job hook `argocd-redis-secret-init`) và hệ thống quan sát ổn định (§6).
...
| ✅ Đã xong | Nguyên | PR #302 đã merge vào `main` |
| ✅ Đã xong | CDO-04 | Merge PR #31 rồi right-size lại ở PR #33 — resources `external-secrets` |
| ✅ Đã xong | CDO-04 | Chạy `helm upgrade` bổ sung resources cho 9 component chính `argocd` (pin `--version 7.3.7`) |
| ⏳ Đang chờ | CDO-04 | Chạy thêm `helm upgrade --reuse-values --set redisSecretInit.resources.*` để đóng gap Job hook `argocd-redis-secret-init` — **bắt buộc trước khi merge PR chuyển `[Deny]`** |
| Review PR evidence + flip Deny | Nguyên | `cdo08/sec-11-argocd-resource-fix-evidence` (`tf4-phase3-repo`) |
...
| Thông báo | tf4-leads | Xác nhận trước khi PR chuyển `[Deny]` được merge/sync (ảnh hưởng `techx-tf4` dùng chung toàn TF) |
...
- [x] Merge PR #302 (`tf4-phase3-repo`) — scope cluster-wide.
- [x] Merge PR #31 + right-size lại PR #33 (gitops) — resources `external-secrets`.
- [x] Chạy `helm upgrade` thủ công cho `argocd` — resources cho 9/9 component chính (evidence: `docs/cdo08/week2/mandate5/evidence/CDO08-SEC-11-argocd-resource-fix-evidence.md`).
- [ ] Chạy `helm upgrade --reuse-values --set redisSecretInit.resources.*` để đóng gap Job hook `argocd-redis-secret-init` — **bắt buộc trước bước merge Deny bên dưới**.
- [ ] Verify trên cluster: 8 object tồn tại, `[Deny]`; bảng compliance §6.1 toàn `✅` (bao gồm Job hook).
- [ ] Notification (§6.2) đã bật: alert Prometheus (deny + error_type) → Alertmanager; ArgoCD `on-sync-failed` → Slack.
- [x] Quan sát qua kênh audit/warn — 0 vi phạm ngoài Job hook `argocd-redis-secret-init` trong suốt thời gian `[Audit, Warn]` (evidence: Prometheus `apiserver_validating_admission_policy_check_total`).
- [ ] Merge PR chuyển `validationActions` sang `[Deny]` (code đã sửa trong `cdo08/sec-11-argocd-resource-fix-evidence`, chờ gate ở §6.1), verify 3 manifest test bị reject thật.
