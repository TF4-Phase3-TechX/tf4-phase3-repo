...
- `techx-observability`: desired manifest của chart hiện tại render sạch cả 4 luật. Điều kiện trước khi enforce thật là ArgoCD phải sync sạch observability sau khi AppProject whitelist đủ `ValidatingAdmissionPolicy`, `ValidatingAdmissionPolicyBinding`, `Namespace` và `StorageClass` (§6.1). Không bật/chấp nhận `[Deny]` dựa trên workload đang vi phạm.
...
| AppProject whitelist | `argocd/root-resources/techx-production.yaml` (repo gitops): `clusterResourceWhitelist` hiện có `ClusterRole`/`ClusterRoleBinding`/webhook config nhưng cần bổ sung `ValidatingAdmissionPolicy`, `ValidatingAdmissionPolicyBinding`, `Namespace`; đồng thời cần `StorageClass` để `techx-observability` sync `gp2-retain`. | ArgoCD chặn sync cluster-scoped kind ngoài whitelist → phải thêm các kind này trước khi enforce cả app + observability (§6). |
...
| **Tin đọc tĩnh `values.yaml` thay vì runtime** | `values.yaml` có thể đã hardening nhưng cluster chưa rollout. | Trước khi bật `[Deny]`, cần cả hai: rendered manifest pass 4 luật và ArgoCD app liên quan `Synced/Healthy`; nếu app còn `OutOfSync/Missing` thì xử lý sync blocker trước. |
...
| Bước | Việc | Kết quả |
|---|---|---|
| 0 | **(Tiên quyết)** Thêm `ValidatingAdmissionPolicy` + `ValidatingAdmissionPolicyBinding` + `Namespace` + `StorageClass` vào `clusterResourceWhitelist` của AppProject `techx-production` (repo gitops) — cần CDO-04 review vì whitelist là project-wide (§2). | ArgoCD được phép sync các kind cluster-scoped này; nếu bỏ bước này, Application `techx-corp` hoặc `techx-observability` sẽ sync fail (`resource ... is not permitted in project`). `StorageClass` là điều kiện để observability sync `gp2-retain`. |
...
| 4 | Theo dõi ngay sau enforce (§6.2) | Nếu bất kỳ workload production nào bị reject ngoài test manifest, rollback rule đó theo §8 và điều tra manifest vi phạm. |
| 5 | Thông báo tf4-leads/Nguyên: admission đã enforce `[Deny]` cả 4 luật trên production | Xác nhận review; mọi bên biết policy đang chặn thật các manifest vi phạm. |
...
| **Resources** | ✅ 0 vi phạm | ✅ 0 vi phạm |
| **Non-root** | ✅ sạch | ✅ desired manifest sạch |
| **Capabilities** | ✅ sạch | ✅ desired manifest sạch |

Không enforce `[Deny]` dựa trên workload đang vi phạm. Với `techx-observability`, điều kiện trước khi coi enforce an toàn là PR GitOps whitelist đã merge, ArgoCD sync được `StorageClass/gp2-retain`, và rendered/live workload pass 4 luật. ExternalSecret conflict của Alertmanager là việc CDO07 xử lý riêng, nhưng nếu conflict làm app `Degraded` thì phải ghi rõ trong change record trước khi rollout.

### 6.2 Observability & Notification — bắt buộc

Vì `[Deny]` là chặn thật ở Kubernetes API server, quan sát là **bắt buộc**, không phải "nên có" — cần biết ngay khi một workload bị reject để rollback kịp. Mỗi `validationActions` surface ra một kênh khác nhau:
...
- `techx-tf4` sạch hoàn toàn — không workload nào bị ảnh hưởng ngoài dự kiến.
- `techx-observability` chỉ được claim đạt khi app đã sync sạch và workload live/rendered pass đủ 4 luật. Nếu app còn `OutOfSync/Missing` vì `StorageClass/gp2-retain` hoặc blocker khác, phải xử lý blocker trước hoặc ghi rõ chưa hoàn tất phần observability.

---

## 8. Rollback / Safety

Rollback path dưới đây cần dùng NGAY nếu một workload thật bị chặn ngoài dự kiến:
...
- Trạng thái: 4 luật `[Deny]` trên `techx-tf4` + `techx-observability`; chỉ claim hoàn tất khi cả rendered manifest và live ArgoCD sync pass.
- Ràng buộc hạ tầng: cần mở rộng `clusterResourceWhitelist` của AppProject `techx-production` cho `ValidatingAdmissionPolicy`, `ValidatingAdmissionPolicyBinding`, `Namespace`, và `StorageClass` (project-wide) — CDO-04 review.
...
| Phụ thuộc còn lại | CDO07 / Platform | CDO07 gỡ xung đột `ExternalSecret`; Platform/ArgoCD sync lại `techx-observability` sau khi whitelist `StorageClass` được merge. Đây là điều kiện để claim observability đã enforce sạch. |
...
- [ ] Mở rộng `clusterResourceWhitelist` của AppProject `techx-production` (repo gitops, PR riêng) cho `ValidatingAdmissionPolicy`, `ValidatingAdmissionPolicyBinding`, `Namespace`, `StorageClass` — CDO-04 duyệt.
...
- [ ] `techx-observability` sync sạch sau khi whitelist `StorageClass/gp2-retain`; rendered/live workload pass đủ 4 luật.
- [ ] Gỡ xung đột `ExternalSecret` với CDO07 hoặc ghi rõ owner/status trong change record nếu conflict chưa ảnh hưởng rollout admission.
