...
Đã cập nhật Helm values để thêm container-level runtime hardening cho các workload có evidence non-root hoặc image nonroot rõ ràng. Không bật `readOnlyRootFilesystem` đại trà.
...
| --- | --- | --- | --- | --- | --- | --- |
| `techx-tf4` | `ad` | `runAsNonRoot`, baseline | Runtime `id` đang `uid=0(root)`; Dockerfile final image `eclipse-temurin:21-jre` chưa set `USER` | Nếu service bị compromise, blast radius cao hơn do root trong container | Nhân + Nam | Rebuild image với user non-root, test JVM/agent write path, sau đó set UID/GID và baseline |
| `techx-tf4` | `cart` | `runAsNonRoot`, baseline | Runtime `id` đang root; Dockerfile final image `mcr.microsoft.com/dotnet/runtime-deps` chưa set `USER` | Root container trong luồng cart có thể tăng tác động khi bị exploit | Nhân + Nam | Thêm non-root user vào Dockerfile hoặc dùng base có user app, test Valkey path/probes |
| `techx-tf4` | `currency` | `runAsNonRoot`, baseline | Runtime `id` đang root; Alpine base chưa set `USER` | Root container trong service gRPC | Nhân + Nam | Build runtime user non-root, kiểm tra binary path `/usr/local/bin/currency` và port |
| `techx-tf4` | `email` | `runAsNonRoot`, baseline | Runtime `id` đang root; Ruby alpine final image chưa set `USER` | Root container trong service email | Nhân + Nam | Thêm user non-root, chown `/email_server` và bundle path nếu cần |
| `techx-tf4` | `flagd` | UID/GID chưa xác nhận | Image không có `id`; có init container copy config vào mounted emptyDir `/etc/flagd` | Ép UID sai có thể làm flagd không đọc/ghi config, ảnh hưởng feature flags | Nhân + Nam | Research official flagd image UID, test init copy ownership, rồi rollout riêng |
| `techx-tf4` | `llm` | `runAsNonRoot`, baseline | Runtime `id` đang root; Python alpine final image chưa set `USER` | Root container cho LLM helper | Nhân + Nam | Thêm app user, chown `/app` và `/venv`, test API |
| `techx-tf4` | `load-generator` | `runAsNonRoot`, baseline | Python/Playwright image thường cần browser/cache paths; chưa audit UID/write paths | Ép non-root có thể làm locust/playwright lỗi ghi cache | Nam | Audit `/opt/pw-browsers`, home/cache paths, thêm user và writable dirs |
| `techx-tf4` | `postgresql` | `runAsNonRoot`, baseline | Official `postgres:17.6` cần audit UID, PVC ownership, `PGDATA`; hiện app user/database config dùng `POSTGRES_USER=root` | Ép sai UID/GID có thể crash DB hoặc làm mất quyền ghi PVC | Nhân + Nam + Nguyên review risk | Test staging với postgres UID, `fsGroup`, PVC ownership; rollout riêng |
| `techx-tf4` | `recommendation` | `runAsNonRoot`, baseline | Python alpine final image chưa set `USER`; chưa audit `/venv` và `/app` ownership | Root container trong recommendation | Nhân + Nam | Thêm app user, chown `/venv`/`/app`, test gRPC |
| `techx-observability` | `opensearch` init `fsgroup-volume` | init container `runAsUser: 0` | Subchart uses root init to `chown` PVC data | Root init increases bootstrap risk but main container is non-root | Nguyên + Nam | Decide if storage class/fsGroup can remove root chown init; not in this task scope |
| `techx-observability` | `alertmanager` | missing `allowPrivilegeEscalation/drop/seccomp` | Already non-root but not explicitly requested in task; can be hardened in follow-up | Lower risk than root, but baseline incomplete | Nhân | Add chart key after validating alertmanager subchart container securityContext |
