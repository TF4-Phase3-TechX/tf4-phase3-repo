...
> Tham số `ScheduleAnyway` đóng vai trò là cơ chế **best-effort spread**. Nó chỉ đạo Kubernetes Scheduler ưu tiên tối đa việc rải Pods sang các AZs khác để đạt `maxSkew: 1`. Nếu không thể thỏa mãn (ví dụ: thiếu tài nguyên, hoặc cấu hình replica thấp trên nút), Scheduler vẫn lập lịch chạy Pod trên AZ có sẵn thay vì giữ Pod ở trạng thái `Pending`.
>
> Do đó, **ScheduleAnyway không đảm bảo tuyệt đối 100% việc phân bổ đều qua các AZs mọi lúc** (ví dụ thực tế là dịch vụ `product-catalog` với 2 replicas đều có thể bị lập lịch chạy trên cùng zone `us-east-1b`). Người vận hành **bắt buộc phải kiểm tra vị trí thực tế của Pods (verify runtime placement)** bằng `kubectl` trước khi demo và không được khẳng định đây là cơ chế phân bổ tuyệt đối.
...
| `product-catalog` | 2 | `ip-10-0-11-101`<br>`ip-10-0-11-217` | `us-east-1b`<br>`us-east-1b` | ⚠️ **Scheduled on us-east-1b only (no zone spread)** |
