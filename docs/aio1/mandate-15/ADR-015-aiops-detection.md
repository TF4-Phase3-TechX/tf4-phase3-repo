...
**Reviewer:** _(Tech Lead signature required before grading)_
...
A single poll exceeding the gate does not produce an incident. The detector requires `AIOPS_SUSTAINED_POLLS` (default: 2) **consecutive** gate violations before creating an incident. This eliminates transient measurement noise.
...
| Period | MTTD measurement method |
|---|---|
| **Before** (manual / alert rules) | Prometheus static threshold rules with `for: 5m`; estimated MTTD ≈ 5–10 minutes |
| **After** (this detector) | `lead_time_steps × poll_seconds` seconds from first incident-window sample to first `breached=true` |

On the labeled dataset, the expected MTTD-after is **≤ 1 poll cycle (45 s)** for acute incidents.
...
- The default thresholds were set conservatively on synthetic data. They must be recalibrated against at least **10 labeled production windows** once the first week of live telemetry is available.
- Thresholds are all `AIOPS_*` environment variables; no code change is needed to recalibrate.
- The replay CLI (`python -m benchmark.replay`) is the canonical recalibration tool.
...
| Role | Name | Date |
|---|---|---|
| Author / AIOps Lead | Đình Thông Trần | 2026-07-21 |
