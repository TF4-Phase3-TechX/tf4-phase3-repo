...
def anomaly_scores(points: list[float]) -> dict[str, float]:
    ...
    baseline, current = points[:-1], points[-1]
    baseline_mean = mean(baseline)
    std = pstdev(baseline) if len(baseline) > 1 else 0.0
    zscore = abs((current - baseline_mean) / std) if std else 0.0
    expected = baseline[0]
    ...
    isolation = 0.0
    if len(points) >= 8 and len(set(points)) >= 3:
        data = np.array(points).reshape(-1, 1)
...
class Detector:
    ...
    def latency(self, service: str, series: list[dict[str, Any]], query: str) -> Decision:
        ...
        breached = current >= self.settings.latency_threshold_ms and (
            scores["ratio"] >= 1.5 or scores["zscore"] >= 3 or scores["ewma"] >= 1
        )
    ...
    def error_rate(self, service: str, series: list[dict[str, Any]], query: str) -> Decision:
        ...
        current = points[-1] if points else 0.0
        scores = anomaly_scores(points)
        breached = current >= self.settings.error_rate_threshold and (
            scores["ratio"] >= 1.5 or scores["zscore"] >= 3 or scores["ewma"] >= 1
        )
    ...
    def llm_error(
        ...
        points = values(series[0]) if series else []
        current = points[-1] if points else 0.0
        breached = current >= self.settings.llm_error_threshold or (log_count is not None and log_count >= 3)
        key = f"llm_timeout_error:{service}"
        self._streaks[key] = self._streaks[key] + 1 if breached else 0
        anomalous = self._streaks[key] >= self.settings.sustained_polls
        torai = torai_lite_score(
            metric=min(current / max(self.settings.llm_error_threshold, 1e-9), 1.0),
        ...
        return Decision(
            ...
            evidence=evidence,
            candidates=[
                {"service": "llm", "score": round(confidence, 3), "signals": torai},
                {"service": "product-reviews", "score": round(confidence * 0.7, 3), "signals": torai},
            ],
...
def llm_error_query(service: str) -> str:
...
def error_rate_query(service: str) -> str:
    return (
